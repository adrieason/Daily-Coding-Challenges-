...
function madShout(sidewalk){
  sidewalk.split()
  let num = [Math.ceil((sidewalk.indexOf('F')-sidewalk.indexOf('Y'))/2)]
  return 'O'+("i").repeat(num)+' F!'
}
...
function towerBuilder(nFloors) {
  let space = (nFloors-1)
    let arr = [(' ').repeat(space)+'*'+(' ').repeat(space)]
  for (i=1;i<nFloors;i++){
    space = space - 1
    arr.push((' ').repeat(space)+('*').repeat(1+(i*2))+(' ').repeat(space))
